...
class TestDeck(unittest.TestCase):
    ...
    def test_shuffle(self):
        ...
        # Make sure they are in a different order, but still the same length
        self.assertNotEqual(notExpected, actual)
        self.assertEqual(len(notExpected), len(actual))
    ...
    def test_valid_bet1(self):
        expected = True
        actual = blackjack.valid_bet("500", 1000)
        self.assertEqual(expected, actual)
        
    def test_valid_bet2(self):
        expected = False
        actual = blackjack.valid_bet("1001", 1000)
        self.assertEqual(expected, actual)

    def test_valid_bet3(self):
        expected = False
        actual = blackjack.valid_bet("0", 1000)
        self.assertEqual(expected, actual)

    def test_calculate_hand_total1(self):
        hand = [{'type': 'Ace', 'value': 11, 'display': 'Ace of Spades'}, {'type': 'number', 'value': 2, 'display': '2 of Spades'}]
        expected = 13
        actual = blackjack.calculate_hand_total(hand)
        self.assertEqual(expected, actual)

    def test_calculate_hand_total2(self):
        hand = [{'type': 'Ace', 'value': 11, 'display': 'Ace of Spades'}, {'type': 'Ace', 'value': 11, 'display': 'Ace of Spades'}, {'type': 'number', 'value': 2, 'display': '2 of Spades'}]
        expected = 14
        actual = blackjack.calculate_hand_total(hand)
        self.assertEqual(expected, actual)

    def test_calculate_hand_total3(self):
        hand = [{'type': 'Ace', 'value': 11, 'display': 'Ace of Spades'}, {'type': 'Ace', 'value': 11, 'display': 'Ace of Clubs'}, {'type': 'Ace', 'value': 11, 'display': 'Ace of Diamonds'}, {'type': 'number', 'value': 2, 'display': '2 of Spades'}]
        expected = 15
        actual = blackjack.calculate_hand_total(hand)
        self.assertEqual(expected, actual)